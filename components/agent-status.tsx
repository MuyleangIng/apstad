...
export function AgentStatus({ connected, showWarning = false }: AgentStatusProps) {
  const testAgentConnection = async () => {
    try {
      const res = await fetch("http://localhost:5001/status", {
        method: "GET",
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm text-muted-foreground">
        {showWarning
          ? "Direct API requests to production domain (no agent needed)"
          : connected
            ? "Local agent is running and ready to proxy requests"
            : "Start the local agent to make requests"}
      </span>
      {!showWarning && (
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={testAgentConnection}>
